...
module.exports = function(host, username, password) {
    ...
    /**
     * Callback for getDeploymentProjectsForPlan
     * @typedef {Function} getDeploymentProjectsForPlanCallback
     */
    /**
     * Returns the list of DeploymentProjectsForPlan
     * @param  {String}   planKey  Given plan to look into
     * @param  {getDeploymentProjectsForPlanCallback} callback
     * @return {Array=}            Array of deployment projects
     */
    Bamboo.prototype.getDeploymentProjectsForPlan = function(planKey = '', callback) {
        const url = this.host + '/rest/api/latest/deploy/project/forPlan.json';
        const planUri = mergeUrlWithParams(url, {planKey});

        request({ uri: planUri }, (error, response, body) =>  {
          const errors = checkErrors(error, response);
     ...
     *
     */
    Bamboo.prototype.getLatestDeploymentEnvironmentStatus = function(environmentId) {

        if (!environmentId) {
            return new Error('No environment passed as an argument');
        }

        const uri = `${this.host}/rest/api/latest/deploy/environment/${environmentId}/results.json`;
        request({ uri }, (error, response, body) =>  {
        const errors = checkErrorsWithResult(error, response);
        ...
        let lastResult = _.first(bodyJson.results);
        callback(null, {
            name: lastResult.deploymentVersionName,
            state: lastResult.deploymentState,
            lifeCycleState:  lastResult.lifeCycleState,
            branchName: lastResult.deploymentVersion.planBranchName
        });
    }
